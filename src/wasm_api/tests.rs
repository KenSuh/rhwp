...
#[test]
fn test_tac_1x1_cell_growth_recomposes_parent_line_height() {
    let mut doc = HwpDocument::create_empty();
    let mut document = Document::default();
    document.sections.push(Section {
    ...
    let before_height = match doc.document.sections[0].paragraphs[0]
        .controls
        .get(control_idx)
    {
        Some(Control::Table(table)) => table.common.height,
        _ => panic!("1x1 TAC 표를 찾을 수 없음"),
    };

    doc.split_paragraph_in_cell_native(0, 0, control_idx, 0, 0, 0)
        .unwrap();

    let after_height = match doc.document.sections[0].paragraphs[0]
        .controls
        .get(control_idx)
    {
        Some(Control::Table(table)) => table.common.height,
    ...
    assert!(
        ...
        "셀 내부 문단 증가 시 1x1 표 높이가 늘어나야 함: before={}, after={}",
        before_height,
        after_height,
    );
    ...
    let recomposed_line_height = doc.composed[0][0]
        ...
        .unwrap_or_default();
    assert!(
        recomposed_line_height >= after_height as i32,
        "TAC 표 높이 증가 후 부모 문단 line_height가 갱신되어야 함: line={}, table={}",
...
#[test]
fn test_tac_1x1_cell_resize_reflows_parent_line_height() {
    let mut doc = HwpDocument::create_empty();
    let mut document = Document::default();
    document.sections.push(Section {
        section_def: crate::model::document::SectionDef {
    ...
    doc.resize_table_cells_native(0, 0, control_idx, r#"[{"cellIdx":0,"heightDelta":3000}]"#)
        .unwrap();
    ...
    assert!(
        after_height > before_height,
        "셀 높이 resize 후 표 높이가 늘어나야 함: before={}, after={}",
        before_height,
        after_height,
    );
    ...
    assert!(
        recomposed_line_height >= after_height as i32,
        "TAC 표 height resize 후 부모 문단 line_height가 갱신되어야 함: line={}, table={}",
        recomposed_line_height,
        after_height,
    );
}
...
fn test_inline_picture_merge_reflows_parent_line_height() {
    ...
    .unwrap();
    doc.insert_text_native(0, 1, 0, "어떤 설비를 말하시는지?")
        .unwrap();
    doc.merge_paragraph_native(0, 1).unwrap();

    let para = &doc.document.sections[0].paragraphs[0];
    assert!(
        matches!(para.controls.first(), Some(Control::Picture(pic)) if pic.common.treat_as_char),
        "병합된 문단에 글자처럼 취급되는 그림이 남아 있어야 함",
    );
    let line_height = para
        ...
        .unwrap_or_default();
    assert!(
        line_height >= picture_height as i32,
        "그림 문단과 다음 문단 병합 후 line_height가 그림 높이 이상이어야 함: line={}, picture={}",
        line_height,
        picture_height,
    );
}
...
#[test]
fn test_nested_wrapper_table_text_layout_preserves_full_cell_path() {
    fn json_usize(json: &str, key: &str) -> usize {
        let pattern = format!("\"{}\":", key);
        let start = json.find(&pattern).expect("key exists") + pattern.len();
        let end = json[start..]
            .find(|c: char| !c.is_ascii_digit())
            .map(|idx| start + idx)
            .unwrap_or(json.len());
        json[start..end].parse().expect("number")
    }
    fn make_path_json(path: &[(usize, usize, usize)]) -> String {
        format!(
            "[{}]",
            path.iter()
                .map(|(ci, cei, cpi)| {
    ...
    doc.set_document(document);
    let outer_json = doc.create_table_native(0, 0, 0, 1, 1).unwrap();
    let parent_para = json_usize(&outer_json, "paraIdx");
    let outer_ctrl = json_usize(&outer_json, "controlIdx");
    let outer_path = vec![(outer_ctrl, 0, 0)];
    let outer_path_json = make_path_json(&outer_path);

    let inner_json = doc
        .create_table_in_cell_by_path_native(0, parent_para, &outer_path_json, 0, 1, 1)
        .unwrap();
    let inner_ctrl = json_usize(&inner_json, "controlIdx");
    let inner_path = vec![(outer_ctrl, 0, 0), (inner_ctrl, 0, 0)];
    let inner_path_json = make_path_json(&inner_path);

    doc.insert_text_in_cell_by_path(0, parent_para, &inner_path, 0, "중첩셀AI결과")
        .unwrap();

    let layout = doc.get_page_text_layout_native(0).unwrap();
    assert!(
        layout.contains("\"text\":\"중첩셀\"") || layout.contains("\"text\":\"중첩셀AI결과\""),
        "중첩 표 셀 텍스트가 렌더 레이아웃에 있어야 함: {}",
        layout,
    );
    assert!(
            layout.contains("\"cellPath\":[{\"controlIndex\":0,\"cellIndex\":0,\"cellParaIndex\":0},{\"controlIndex\":0,\"cellIndex\":0,\"cellParaIndex\":0}]"),
            "1x1 래퍼 표를 건너뛰더라도 중첩 cellPath depth=2가 보존되어야 함: {}",
            layout,
        );

    let bboxes = doc.get_table_cell_bboxes_by_path_native(0, parent_para, &inner_path_json);
    assert!(
        bboxes.is_ok(),
        "중첩 표 bbox 조회가 full cellPath로 가능해야 함: {:?}",
        bboxes.err(),
    );
}

#[test]
fn test_hit_test_nested_table_uses_innermost_cell_after_split_cell_paragraph() {
    fn json_usize(json: &str, key: &str) -> usize {
        let pattern = format!("\"{}\":", key);
        let start = json.find(&pattern).expect("key exists") + pattern.len();
        let end = json[start..]
    ...
    fn nth_cell_bbox_json(cells_json: &str, nth: usize) -> &str {
        let mut index = 0usize;
        let mut start = None;
        for (pos, ch) in cells_json.char_indices() {
            if ch == '{' {
                if index == nth {
                    start = Some(pos);
                    break;
        ...
        let start = start.expect("bbox object exists");
        let end = cells_json[start..]
            .find('}')
            .map(|i| start + i + 1)
            .unwrap();
        &cells_json[start..end]
    }
    ...
    doc.insert_text_native(0, 0, 0, "본문 앞").unwrap();
    let outer_json = doc.create_table_native(0, 0, 4, 1, 1).unwrap();
    let parent_para = json_usize(&outer_json, "paraIdx");
    let outer_ctrl = json_usize(&outer_json, "controlIdx");
    let outer_path = vec![(outer_ctrl, 0, 0)];
    doc.insert_text_in_cell_by_path(0, parent_para, &outer_path, 0, "첫 문단 텍스트")
        .unwrap();
    let split_json = doc
        .split_paragraph_in_cell_by_path(
            0,
            parent_para,
            &outer_path,
            "첫 문단 텍스트".chars().count(),
        )
        .unwrap();
    let split_cell_para_idx = json_usize(&split_json, "cellParaIndex");
    let outer_second_para_path = vec![(outer_ctrl, 0, split_cell_para_idx)];
    let outer_second_para_path_json = make_path_json(&outer_second_para_path);

    let nested_json = doc
        .create_table_in_cell_by_path_native(0, parent_para, &outer_second_para_path_json, 0, 1, 2)
        .unwrap();
    let nested_ctrl = json_usize(&nested_json, "controlIdx");
    let first_inner_path = vec![(outer_ctrl, 0, split_cell_para_idx), (nested_ctrl, 0, 0)];
    let second_inner_path = vec![(outer_ctrl, 0, split_cell_para_idx), (nested_ctrl, 1, 0)];
    let first_inner_path_json = make_path_json(&first_inner_path);
    doc.insert_text_in_cell_by_path(0, parent_para, &first_inner_path, 0, "A")
        .unwrap();
    doc.insert_text_in_cell_by_path(0, parent_para, &second_inner_path, 0, "B")
        .unwrap();

    let bboxes = doc
        .get_table_cell_bboxes_by_path_native(0, parent_para, &first_inner_path_json)
        .unwrap();
    let first_box = nth_cell_bbox_json(&bboxes, 0);
    let second_box = nth_cell_bbox_json(&bboxes, 1);
    let first_hit_x = json_f64(first_box, "x") + json_f64(first_box, "w") / 2.0;
    let first_hit_y = json_f64(first_box, "y") + json_f64(first_box, "h") / 2.0;
    let second_hit_x = json_f64(second_box, "x") + json_f64(second_box, "w") / 2.0;
    let second_hit_y = json_f64(second_box, "y") + json_f64(second_box, "h") / 2.0;

    let first_hit = doc.hit_test_native(0, first_hit_x, first_hit_y).unwrap();
    assert!(
        first_hit.contains("\"cellPath\":[{\"controlIndex\":0,\"cellIndex\":0,\"cellParaIndex\":1},{\"controlIndex\":0,\"cellIndex\":0,\"cellParaIndex\":0}]"),
        "첫 번째 안쪽 셀 중앙 클릭은 형제 셀이 아니라 첫 번째 안쪽 셀을 반환해야 함: {}",
        first_hit,
    );

    let second_hit = doc.hit_test_native(0, second_hit_x, second_hit_y).unwrap();
    assert!(
        second_hit.contains("\"cellPath\":[{\"controlIndex\":0,\"cellIndex\":0,\"cellParaIndex\":1},{\"controlIndex\":0,\"cellIndex\":1,\"cellParaIndex\":0}]"),
        "두 번째 안쪽 셀 중앙 클릭은 두 번째 안쪽 셀을 반환해야 함: {}",
        second_hit,
    );
}
...
fn test_task105_nested_table_path_api() {
    ...
    let path_json = make_path_json(&path);
    ...
    // 8. 중첩 표 셀 안에서 다시 표를 만들고, 그 안쪽 표에 텍스트/구조 변경이 들어가야 한다.
    let insert = doc.create_table_in_cell_by_path_native(0, parent_para, &path_json, 0, 1, 2);
    assert!(
        insert.is_ok(),
        "createTableInCellByPath 실패: {:?}",
        insert.err()
    );
    let insert_json = insert.unwrap();
    let inner_ctrl = json_usize(&insert_json, "controlIdx");
    let mut child_path = path.clone();
    child_path.push((inner_ctrl, 0, 0));
    let child_path_json = make_path_json(&child_path);

    let child_dims = doc.get_table_dimensions_by_path_native(0, parent_para, &child_path_json);
    assert!(
        child_dims.is_ok(),
        "생성된 안쪽 표 조회 실패: {:?}",
        child_dims.err()
    );
    let child_dims_json = child_dims.unwrap();
    assert!(
        child_dims_json.contains("\"rowCount\":1"),
        "안쪽 표 rowCount=1: {}",
        child_dims_json
    );
    assert!(
        child_dims_json.contains("\"colCount\":2"),
        "안쪽 표 colCount=2: {}",
        child_dims_json
    );

    doc.insert_text_in_cell_by_path(0, parent_para, &child_path, 0, "AI")
        .unwrap();
    let text = doc
        .get_text_in_cell_by_path(0, parent_para, &child_path, 0, 2)
        .unwrap();
    assert_eq!(text, "AI", "생성된 안쪽 표 셀에 텍스트가 삽입되어야 함");

    let child_height_before_split = doc
        .resolve_table_by_path(0, parent_para, &child_path)
        .unwrap()
        .common
        .height;
    let split_result = doc
        .split_paragraph_in_cell_by_path(0, parent_para, &child_path, 2)
        .unwrap();
    let split_cell_para_idx = json_usize(&split_result, "cellParaIndex");
    let mut child_second_para_path = child_path.clone();
    child_second_para_path.last_mut().unwrap().2 = split_cell_para_idx;
    doc.insert_text_in_cell_by_path(
        0,
        parent_para,
        &child_second_para_path,
        0,
        "중첩 표 내부에서 늘어난 문단",
    )
    .unwrap();
    let child_height_after_split = doc
        .resolve_table_by_path(0, parent_para, &child_path)
        .unwrap()
        .common
        .height;
    assert!(
        child_height_after_split > child_height_before_split,
        "중첩 표 셀 문단 증가 시 안쪽 표 높이가 함께 늘어나야 함: before={}, after={}",
        child_height_before_split,
        child_height_after_split,
    );

    doc.insert_table_column_by_path_native(0, parent_para, &child_path_json, 0, true)
        .unwrap();
    let after_col = doc
        .get_table_dimensions_by_path_native(0, parent_para, &child_path_json)
        .unwrap();
    assert!(
        after_col.contains("\"colCount\":3"),
        "안쪽 표 열 추가가 안쪽 표에 적용되어야 함: {}",
        after_col
    );

    doc.copy_control_by_path_native(0, parent_para, &child_path_json)
        .unwrap();
    assert_eq!(
        doc.get_clipboard_text_native(),
        "[표]",
        "안쪽 표 복사도 표 개체로 인식되어야 함"
    );
    let copied_table_cols = doc
        .clipboard
        .as_ref()
        .and_then(|clip| clip.paragraphs.first())
        .and_then(|para| para.controls.first())
        .and_then(|control| match control {
            crate::model::control::Control::Table(table) => Some(table.col_count),
            _ => None,
        });
    assert_eq!(
        copied_table_cols,
        Some(3),
        "중첩 표 복사는 바깥 표가 아니라 선택한 안쪽 표를 복사해야 함"
    );

    let pasted = doc
        .paste_control_in_cell_by_path_native(0, parent_para, &child_path_json, 2)
        .unwrap();
    let pasted_ctrl = json_usize(&pasted, "controlIdx");
    let mut pasted_path = child_path.clone();
    pasted_path.push((pasted_ctrl, 0, 0));
    let pasted_path_json = make_path_json(&pasted_path);
    let pasted_dims = doc
        .get_table_dimensions_by_path_native(0, parent_para, &pasted_path_json)
        .unwrap();
    assert!(
        pasted_dims.contains("\"colCount\":3"),
        "중첩 표 셀 안에 붙여넣은 표가 같은 열 수를 유지해야 함: {}",
        pasted_dims
    );

    let html_table = r#"<table><tr><td>H</td><td>I</td></tr></table>"#;
    let html_result = doc
        .paste_html_in_cell_by_path_native(0, parent_para, &child_path_json, 2, html_table)
        .unwrap();
    let html_cell_para_idx = json_usize(&html_result, "cellParaIdx");
    let mut html_base_path = child_path.clone();
    html_base_path.last_mut().unwrap().2 = html_cell_para_idx;
    let mut html_table_path = html_base_path.clone();
    html_table_path.push((0, 0, 0));
    let html_table_path_json = make_path_json(&html_table_path);
    let html_dims = doc
        .get_table_dimensions_by_path_native(0, parent_para, &html_table_path_json)
        .unwrap();
    assert!(
        html_dims.contains("\"rowCount\":1"),
        "HTML 표가 중첩 셀 안에 생성되어야 함: {}",
        html_dims
    );
    assert!(
        html_dims.contains("\"colCount\":2"),
        "HTML 표 열 수가 보존되어야 함: {}",
        html_dims
    );

    doc.set_cell_properties_by_path_native(
        0,
        parent_para,
        &child_path_json,
        "{\"height\":1234,\"paddingLeft\":77}",
    )
    .unwrap();
    let child_cell_props = doc
        .get_cell_properties_by_path_native(0, parent_para, &child_path_json)
        .unwrap();
    assert!(
        child_cell_props.contains("\"height\":1234"),
        "안쪽 표 셀 속성이 안쪽 셀에 적용되어야 함: {}",
        child_cell_props
    );
    assert!(
        child_cell_props.contains("\"paddingLeft\":77"),
        "안쪽 표 셀 여백이 안쪽 셀에 적용되어야 함: {}",
        child_cell_props
    );

    doc.set_table_properties_by_path_native(
        0,
        parent_para,
        &child_path_json,
        "{\"cellSpacing\":9,\"paddingLeft\":55}",
    )
    .unwrap();
    let child_table_props = doc
        .get_table_properties_by_path_native(0, parent_para, &child_path_json)
        .unwrap();
    assert!(
        child_table_props.contains("\"cellSpacing\":9"),
        "안쪽 표 속성이 안쪽 표에 적용되어야 함: {}",
        child_table_props
    );
    assert!(
        child_table_props.contains("\"paddingLeft\":55"),
        "안쪽 표 안쪽여백이 안쪽 표에 적용되어야 함: {}",
        child_table_props
    );

    let flat_html_result = doc
        .paste_html_in_cell_native(
            ...
            path[0].2,
            0,
            r#"<table><tr><td>F</td><td>G</td></tr></table>"#,
        )
        .unwrap();
    let _flat_html_cell_para_idx = json_usize(&flat_html_result, "cellParaIdx");
    let flat_cell = doc
        .resolve_cell_by_path(0, parent_para, &[path[0]])
        .unwrap();
    let has_flat_html_table = flat_cell.paragraphs.iter().any(|para| {
        para.controls.iter().any(|control| {
            matches!(
                control,
                Control::Table(table) if table.row_count == 1 && table.col_count == 2
            )
        })
    });
    assert!(
        has_flat_html_table,
        "일반 표 셀 안 HTML 표도 셀 내부 표로 보존되어야 함"
    );

    // 9. hitTest에서 cellPath 포함 확인
...
#[test]
fn test_nested_table_picture_path_properties_and_delete() {
    let mut doc = HwpDocument::create_empty();
    ...
    let outer = doc.create_table_native(0, 0, 0, 1, 1).unwrap();
    let parent_para = json_usize(&outer, "paraIdx");
    let outer_ctrl = json_usize(&outer, "controlIdx");
    let outer_path = vec![(outer_ctrl, 0, 0)];
    let outer_path_json = make_path_json(&outer_path);
    let nested = doc
        .create_table_in_cell_by_path_native(0, parent_para, &outer_path_json, 0, 1, 1)
        .unwrap();
    let nested_ctrl = json_usize(&nested, "controlIdx");
    let path = vec![(outer_ctrl, 0, 0), (nested_ctrl, 0, 0)];
    let path_json = make_path_json(&path);
    ...
    let insert = doc
        .insert_picture_in_cell_by_path_native(
            0,
            parent_para,
            &path_json,
            0,
            ...
            1,
            "png",
            "nested-picture-path-regression",
        )
        .unwrap();
    let control_idx = json_usize(&insert, "controlIdx");

    assert!(
        doc.get_picture_properties_native(0, parent_para, control_idx)
            .is_err(),
        "중첩 셀 그림은 바깥 문단 controls[controlIdx]로 조회되면 안 됨",
    );

    let props = doc
        .get_picture_properties_by_path_native(0, parent_para, &path_json, control_idx)
        .unwrap();
    assert!(
        props.contains("\"width\":2200"),
        "삽입 폭이 path API로 조회되어야 함: {}",
        props
    );
    assert!(
        props.contains("\"height\":1300"),
        "삽입 높이가 path API로 조회되어야 함: {}",
        props
    );

    doc.set_picture_properties_by_path_native(
        0,
        parent_para,
        &path_json,
        control_idx,
        "{\"width\":2900,\"height\":1600,\"rotationAngle\":15}",
    )
    .unwrap();
    let resized = doc
        .get_picture_properties_by_path_native(0, parent_para, &path_json, control_idx)
        .unwrap();
    assert!(
        resized.contains("\"width\":2900"),
        "path 리사이즈 폭이 반영되어야 함: {}",
        resized
    );
    assert!(
        resized.contains("\"height\":1600"),
        "path 리사이즈 높이가 반영되어야 함: {}",
        resized
    );
    assert!(
        resized.contains("\"rotationAngle\":15"),
        "path 회전 속성이 반영되어야 함: {}",
        resized
    );

    let layout = doc.get_page_control_layout_native(0).unwrap();
    assert!(
        layout.contains("\"type\":\"image\""),
        "컨트롤 layout에 이미지가 있어야 함: {}",
        layout
    );
    assert!(
        layout.contains("\"cellPath\""),
        "중첩 표 이미지 layout은 cellPath를 포함해야 함: {}",
        layout
    );
    assert!(
        layout.contains("\"parentParaIdx\""),
        "중첩 표 이미지 layout은 부모 문단을 포함해야 함: {}",
        layout
    );

    let control_path = vec![(outer_ctrl, 0, 0), (nested_ctrl, 0, 0), (control_idx, 0, 0)];
    let control_path_json = make_path_json(&control_path);
    let html = doc
        .export_control_html_by_path_native(0, parent_para, &control_path_json)
        .unwrap();
    assert!(
        html.contains("<img") && html.contains("data:image/png"),
        "중첩 셀 그림은 path 기반 HTML 내보내기가 가능해야 함: {}",
        html
    );
    let image_data = doc
        .get_control_image_data_by_path_native(0, parent_para, &control_path_json)
        .unwrap();
    assert_eq!(
        image_data, PNG_1X1,
        "중첩 셀 그림의 원본 이미지 데이터가 path API로 조회되어야 함",
    );
    let image_mime = doc
        .get_control_image_mime_by_path_native(0, parent_para, &control_path_json)
        .unwrap();
    assert_eq!(
        image_mime, "image/png",
        "중첩 셀 그림 MIME이 path API로 조회되어야 함",
    );
    doc.copy_control_by_path_native(0, parent_para, &control_path_json)
        .unwrap();
    assert!(
        doc.clipboard_has_control_native(),
        "중첩 셀 그림은 대상 controlIndex를 포함한 path로 내부 클립보드 복사가 가능해야 함",
    );

    doc.delete_picture_control_by_path_native(0, parent_para, &path_json, control_idx)
        .unwrap();
    let target_para = doc
        .resolve_paragraph_by_path(0, parent_para, &path)
        .unwrap();
    let deleted = target_para
        .controls
        .get(control_idx)
        .map(|control| matches!(control, Control::Picture(_)))
        .unwrap_or(false);
    assert!(
        !deleted,
        "path 삭제 후 해당 중첩 셀 문단의 그림 컨트롤이 제거되어야 함"
    );
}
