...
impl Paginator {
    ...
    fn paginate_table_control(
            ...
            && table.common.vertical_offset > 0
        {
            ...
            let abs_bottom = para_start_height
                + v_off
                + effective_height
                + host_spacing
        ...
        if let Some(mp) = measured.get_measured_paragraph(para_idx) {
            ...
            if is_independent_float {
                let v_off =
                    crate::renderer::hwpunit_to_px(table.common.vertical_offset as i32, self.dpi);
                let offset_bottom =
                    para_start_height + v_off + effective_height + caption_extra_for_current;
                let flow_bottom =
                    st.current_height + table_total_height + caption_extra_for_current;
                let float_bottom = offset_bottom.max(flow_bottom);
        ...
        while cursor_row < row_count {
...

            {
                ...
                if approx_end <= cursor_row {
                    ...
                    } else if can_intra_split && effective_first_row_h > base_available_height {
                        // 분할 불가능한 행은 원자적으로 다음 페이지로 넘긴다.
                        // 단, 행 자체가 한 페이지보다 큰 경우에만 가용 높이에 맞춰 강제 분할한다.
                        let padding = mt.max_padding_for_row(r);
                        let avail_content = (avail_for_rows - padding).max(0.0);
                        let snapped_content = mt.snap_nested_row_split_limit(r, avail_content);
                        if snapped_content >= MIN_SPLIT_CONTENT_PX {
                            end_row = r + 1;
                            split_end_limit = snapped_content;
                        } else {
                            end_row = r + 1;
                        }
                    } else {
                    ...
                    if can_intra_split && mt.is_row_splittable(r) {
                        ...
                        let avail_content_for_r = (remaining_avail - row_cs - padding).max(0.0);
                        let snapped_content =
                            mt.snap_nested_row_split_limit(r, avail_content_for_r);
                        if snapped_content >= MIN_SPLIT_CONTENT_PX {
                            end_row = r + 1;
                            split_end_limit = snapped_content;
                        }
