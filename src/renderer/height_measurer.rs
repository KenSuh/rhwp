    ...
    fn measure_table_impl(
        ...
        for cell in &table.cells {
            if cell.row_span == 1 && (cell.row as usize) < row_count {
                ...
                let nested_table_height =
                    self.cell_controls_height(&cell.paragraphs, styles, depth);
                let content_height = if has_nested_table_in_cell {
                    ...
                    let last_seg_end: i32 = cell
                        ...
                        .max()
                        .unwrap_or(0);
                    hwpunit_to_px(last_seg_end, self.dpi).max(text_height + nested_table_height)
                } else {
        ...
        for cell in &table.cells {
            ...
            if span > 1 && r + span <= row_count {
                ...
                let nested_table_height =
                    self.cell_controls_height(&cell.paragraphs, styles, depth);
                let content_height = text_height + non_inline_h + nested_table_height;
                let required_height = content_height + pad_top + pad_bottom;
...
impl MeasuredTable {
    ...
    pub fn is_row_splittable(&self, row: usize) -> bool {
        ...
        if cells_in_row.iter().any(|c| c.has_nested_table) {
            return false;
        }
        cells_in_row.iter().any(|c| c.line_heights.len() > 1)
    }

    /// 지정 행에 중첩 표가 포함되어 있는지 판별한다.
    pub fn row_has_nested_table(&self, row: usize) -> bool {
        self.cells
            .iter()
            .filter(|c| c.row == row && c.row_span == 1)
            .any(|c| c.has_nested_table)
    }

    /// 중첩 표가 있는 행의 분할 지점을 줄 경계로 보정한다.
    ///
    /// 긴 셀 안에서 중첩 표가 페이지 하단에 걸리면 표 일부만 보이는 조각이 생긴다.
    /// proposed_limit가 특정 줄 내부에 걸리는 경우, 현재 페이지에는 마지막으로 완전히
    /// 들어간 줄까지만 남기고 다음 fragment에서 중첩 표를 온전하게 배치하게 한다.
    pub fn snap_nested_row_split_limit(&self, row: usize, proposed_limit: f64) -> f64 {
        if proposed_limit <= 0.0 || !self.row_has_nested_table(row) {
            return proposed_limit;
        }

        let mut snapped = proposed_limit;
        let mut found_boundary = false;
        for cell in self
            .cells
            .iter()
            .filter(|c| c.row == row && c.row_span == 1 && c.has_nested_table)
        {
            let mut boundary = 0.0;
            let mut acc = 0.0;
            for &line_h in &cell.line_heights {
                let next = acc + line_h;
                if next <= proposed_limit + 0.5 {
                    boundary = next;
                } else {
                    break;
                }
