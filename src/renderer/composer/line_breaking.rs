...
pub(crate) fn reflow_line_segs(
    ...
    // 글자처럼 취급되는 인라인 컨트롤의 높이 반영.
    // 표뿐 아니라 그림/도형/수식도 Backspace 문단 병합 후 같은 줄의 line_height를 확보해야 한다.
    {
        use crate::model::control::Control;
        let max_inline_control_height = para
            .controls
            .iter()
            ...
            .max()
            .unwrap_or(0);
        if max_inline_control_height > 0 {
            if let Some(seg) = new_line_segs.first_mut() {
                if max_inline_control_height > seg.line_height {
                    seg.line_height = max_inline_control_height;
                    seg.text_height = max_inline_control_height;
                    seg.baseline_distance = (max_inline_control_height as f64 * 0.85) as i32;
                }
            }
        }
    }
