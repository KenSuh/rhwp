...
pub(crate) fn render_edge_borders(
    ...
    if row_col_x.is_empty() || row_y.is_empty() {
        return nodes;
    }
    let row_count = if row_y.len() > 1 { row_y.len() - 1 } else { 0 };
    let edge_x = |xs: &[f64], index: usize| -> Option<f64> {
        xs.get(index).copied().or_else(|| xs.last().copied())
    };
    ...
    for (ri, h_row) in h_edges.iter().enumerate() {
                ...
                if same_style {
                    ...
                    if let (Some(start), Some(ref sb)) = (seg_start, seg_border) {
                        if let (Some(start_x), Some(end_x)) =
                            (edge_x(ref_cx, start), edge_x(ref_cx, ci))
                        {
                            let x1 = table_x + start_x;
                            let x2 = table_x + end_x;
        ...
        // 마지막 세그먼트
        if let (Some(start), Some(ref sb)) = (seg_start, seg_border) {
            if let Some(start_x) = edge_x(ref_cx, start) {
                let x1 = table_x + start_x;
                let x2 = table_x + edge_x(ref_cx, h_row.len()).unwrap_or(start_x);
                nodes.extend(create_border_line_nodes(tree, &sb, x1, y, x2, y));
            }
        }
    }
    ...
    for (ci, v_col) in v_edges.iter().enumerate() {
                ...
                if same_style {
                    ...
                    if let (Some(start), Some(ref sb)) = (seg_start, seg_border) {
                        if let (Some(start_y), Some(end_y)) =
                            (row_y.get(start).copied(), row_y.get(ri).copied())
                        {
                            let y1 = table_y + start_y;
                            let y2 = table_y + end_y;
        ...
        if let (Some(start), Some(ref sb)) = (seg_start, seg_border) {
            if let Some(start_y) = row_y.get(start).copied() {
                let y1 = table_y + start_y;
                let y2 = table_y + row_y.get(v_col.len()).copied().unwrap_or(start_y);
                nodes.extend(create_border_line_nodes(tree, &sb, seg_x, y1, seg_x, y2));
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn render_edge_borders_tolerates_empty_coordinate_grid() {
        let mut tree = PageRenderTree::new(0, 100.0, 100.0);
        let h_edges = vec![vec![Some(BorderLine::default())]];
        let nodes = render_edge_borders(&mut tree, &h_edges, &[], &[], &[], 0.0, 0.0);
