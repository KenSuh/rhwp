...
impl LayoutEngine {
    ...
    pub(crate) fn layout_partial_table(
        ...
        for (cell_idx, cell) in table.cells.iter().enumerate() {
            ...
            if is_split_end_row {
                let has_visible_split_content = line_ranges
                    .as_ref()
                    .map(|ranges| {
                        ranges.iter().enumerate().any(|(idx, &(start, end))| {
                            start < end
                                || cell
                                    .paragraphs
                                    .get(idx)
                                    .map(|p| p.controls.iter().any(|c| matches!(c, Control::Table(_))))
                                    .unwrap_or(false)
                        })
                    })
                    .unwrap_or(true);
                if !has_visible_split_content {
                    continue;
                }
            }
