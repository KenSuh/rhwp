...
impl LayoutEngine {
    ...
    pub(crate) fn layout_table(
        ...
        if table.row_count == 1 && table.col_count == 1 && table.cells.len() == 1 {
            ...
            if !has_visible_text {
                if let Some((cell_para_idx, nested_ctrl_idx, nested)) =
                    cell.paragraphs.iter().enumerate().find_map(|(cp_idx, p)| {
                        p.controls.iter().enumerate().find_map(|(ctrl_idx, c)| {
                            if let Control::Table(t) = c {
                                Some((cp_idx, ctrl_idx, t.as_ref()))
                            } else {
                                None
                            }
                        })
                ...
                {
                    let nested_cell_ctx = if let Some(mut ctx) = enclosing_cell_ctx.clone() {
                        if let Some(last) = ctx.path.last_mut() {
                            last.cell_index = 0;
                            last.cell_para_index = cell_para_idx;
    ...
    pub(crate) fn resolve_row_heights(
        ...
        for cell in &table.cells {
            if cell.row_span == 1 && (cell.row as usize) < row_count {
                ...
                let base_content_height = if cell.text_direction != 0 {
                ...
                let content_height =
                    base_content_height + self.calc_cell_controls_height(cell, styles);
        ...
        for cell in &table.cells {
            ...
            if span > 1 && r + span <= row_count {
                ...
                let content_height = self
                    .calc_cell_paragraphs_content_height(&cell.paragraphs, styles)
                    + self.calc_cell_controls_height(cell, styles);
                let required_height = content_height + pad_top + pad_bottom;
