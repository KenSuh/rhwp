...
fn decode_general_ref(reference: &BytesRef<'_>) -> Option<String> {
    ...
    match value {
        ...
        _ if value.starts_with('#') => value[1..]
            .parse::<u32>()
            .ok()
            .and_then(char::from_u32)
            .map(|c| c.to_string()),
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::GeneralRef(ref reference)) => {
                if let Some(value) = decode_general_ref(reference) {
                    text.push_str(&value);
                }
            }
            Ok(Event::End(ref e)) => {
    ...
    loop {
        match reader.read_event_into(&mut buf) {
            ...
            Ok(Event::Text(ref txt)) => {
                if in_script {
                    script.push_str(&decode_text(txt));
                }
            }
            Ok(Event::GeneralRef(ref reference)) => {
                if in_script {
                    ...
                    b"text" => {
                        ...
                        loop {
                            match reader.read_event_into(&mut tbuf) {
                                Ok(Event::Text(ref t)) => {
                                    form.text.push_str(&decode_text(t));
                                }
                                Ok(Event::GeneralRef(ref reference)) => {
                                    if let Some(value) = decode_general_ref(reference) {
                                        form.text.push_str(&value);
                                    }
                                }
