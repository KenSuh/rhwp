...
impl DocumentCore {
    ...
    pub fn hit_test_native(&self, page_num: u32, x: f64, y: f64) -> Result<String, HwpError> {
        ...
        fn collect_runs(
            ...
            let table_depth = if matches!(node.node_type, RenderNodeType::Table(_)) {
        ...
        if let Some(cb) = clicked_cell {
                    ...
                    r.cell_context
                        ...
                        .map(|ctx| {
                            ctx.parent_para_index == cb.parent_para_index
                                && ctx.path.len() == cb.table_depth
                                && ctx
                                    .path
                                    .first()
                                    .map(|e| e.control_index == cb.control_index)
                                    .unwrap_or(false)
                                && ctx
                                    .path
                                    .last()
                                    .map(|e| e.cell_index == cb.cell_index)
                                    .unwrap_or(false)
                        })
                        .unwrap_or(false)
                })
            ...
            if cb.table_depth > 1 {
                if let Some(ctx) = runs.iter().find_map(|r| {
                    let ctx = r.cell_context.as_ref()?;
                    if ctx.parent_para_index == cb.parent_para_index
                        && ctx.path.len() == cb.table_depth
                        && ctx
                            .path
                            .first()
                    ...
                    return Ok(format!(
                        ...
                        cb.parent_para_index, outer.control_index, outer.cell_index, outer.cell_para_index,
                        path_entries.join(","),
                        page_num,
