...
impl DocumentCore {
    ...
    /// cellPath 기반 셀 내부 문단의 글자 속성 조회 (중첩 표 지원)
    pub fn get_cell_char_properties_at_by_path_native(
        &self,
        sec_idx: usize,
        parent_para_idx: usize,
    ...
    /// cellPath 기반 셀 내부 문단의 문단 속성 조회 (네이티브)
    pub fn get_cell_para_properties_at_by_path_native(
        &self,
        sec_idx: usize,
        parent_para_idx: usize,
        path: &[(usize, usize, usize)],
        ...
        let para = self
            .get_cell_paragraph_ref_by_path(sec_idx, parent_para_idx, path)
            .ok_or_else(|| HwpError::RenderError("cellPath 셀 문단을 찾을 수 없음".to_string()))?;
        Ok(self.build_para_properties_json(para.para_shape_id, sec_idx))
    }
    ...
    /// 글자 서식 적용 (네이티브) — cellPath 기반 셀 내 문단
    pub fn apply_char_format_in_cell_by_path_native(
        ...
        if mods.base_size.is_some() {
            self.reflow_cell_paragraph_by_path(sec_idx, parent_para_idx, path);
        }
        self.refresh_tables_after_cell_path_edit(sec_idx, parent_para_idx, path);
        self.document.sections[sec_idx].raw_stream = None;
        self.rebuild_section(sec_idx);
    ...
    /// 문단 서식 적용 (네이티브) — cellPath 기반 셀 내 문단
