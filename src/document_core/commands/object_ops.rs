...
impl DocumentCore {
    ...
    /// 셀/중첩 셀 내부 그림 컨트롤의 기본 속성을 cellPath 기준으로 조회한다.
    ///
    /// 객체 선택/리사이즈/이동 핸들에서 필요한 핵심 속성만 반환한다.
    pub fn get_picture_properties_by_path_native(
        &self,
        section_idx: usize,
        parent_para_idx: usize,
        path_json: &str,
    ...
    /// 셀/중첩 셀 내부 그림 컨트롤의 속성을 cellPath 기준으로 변경한다.
    pub fn set_picture_properties_by_path_native(
        &mut self,
        section_idx: usize,
        parent_para_idx: usize,
        path_json: &str,
    ...
    /// 셀/중첩 셀 내부 그림 컨트롤을 cellPath 기준으로 삭제한다.
    pub fn delete_picture_control_by_path_native(
        &mut self,
        section_idx: usize,
        parent_para_idx: usize,
        path_json: &str,
        ...
        let dpi = self.dpi;
        {
            ...
            for (i, &(table_ctrl_idx, cell_idx, cell_para_idx)) in path.iter().enumerate() {
                ...
                if i == path.len() - 1 {
                    if control_idx >= cell_para.controls.len() {
                        return Err(HwpError::RenderError(format!(
                            "컨트롤 인덱스 {} 범위 초과",
                            control_idx
                        )));
                    }
                    if !matches!(&cell_para.controls[control_idx], Control::Picture(_)) {
                        return Err(HwpError::RenderError(
                            "지정된 컨트롤이 그림이 아닙니다".to_string(),
                        ));
                    }
                    ...
                    let mut prev_end: u32 = 0;
                    let mut gap_start: Option<u32> = None;
                    'outer: for char_idx in 0..text_chars.len() {
                        let offset = if char_idx < cell_para.char_offsets.len() {
                            cell_para.char_offsets[char_idx]
                        } else {
                            prev_end
                        };
                        while prev_end + 8 <= offset && ci < cell_para.controls.len() {
                            if ci == control_idx {
                                gap_start = Some(prev_end);
                                break 'outer;
                            }
                        ...
                        let char_size: u32 = if text_chars[char_idx] == '\t' {
                            8
                        } else if text_chars[char_idx].len_utf16() == 2 {
                            2
                        } else {
                            1
                        };
                    ...
                    if gap_start.is_none() {
                        while ci < cell_para.controls.len() {
                            if ci == control_idx {
                                gap_start = Some(prev_end);
                                break;
                            }
                    ...
                    if let Some(gs) = gap_start {
                        let threshold = gs + 8;
                        for offset in cell_para.char_offsets.iter_mut() {
                            if *offset >= threshold {
                                *offset -= 8;
                            }
                        }
                    ...
                    cell_para.controls.remove(control_idx);
                    if control_idx < cell_para.ctrl_data_records.len() {
                        cell_para.ctrl_data_records.remove(control_idx);
                    }
                    if cell_para.char_count >= 8 {
                        cell_para.char_count -= 8;
                    }
                    Self::reflow_paragraph_line_segs_after_control_delete(cell_para, styles, dpi);
                    break;
                }
    ...
    /// 셀/중첩 셀 내부 커서 위치에 인라인 표를 삽입한다.
    ///
    /// 일반 createTable은 본문 문단 배열에 새 문단을 삽입한다. 셀 안에서 그 API를
    /// 호출하면 parentParaIndex가 아닌 paragraphIndex를 기준으로 들어가서 표 밖에
    /// 생성된다. 이 경로는 cellPath로 최종 셀 문단을 찾아 그 문단 controls에 표를
    /// 직접 추가한다.
    pub fn create_table_in_cell_by_path_native(
        &mut self,
        section_idx: usize,
        parent_para_idx: usize,
        path_json: &str,
        ...
        let path = Self::parse_cell_path(path_json)?;
        let (content_width, default_char_shape_id, default_para_shape_id) = {
            ...
            let para = cell.paragraphs.get(target_para_idx).ok_or_else(|| {
                HwpError::RenderError(format!("셀문단 {} 범위 초과", target_para_idx))
            })?;
            let inner_width =
                cell.width as i32 - cell.padding.left as i32 - cell.padding.right as i32 - 566; // 좌우 outer margin 기본값(283*2) 보정
            (
                inner_width.max(1200) as u32,
                para.char_shapes
                    .first()
                    .map(|cs| cs.char_shape_id)
                    .unwrap_or(0),
        ...
        let (table, total_height) = self.build_inline_table_control(
            row_count,
            col_count,
            content_width,
        ...
        let ctrl_idx = {
            ...
            Self::insert_inline_control_into_paragraph(
                target_para,
        ...
        let outer_ctrl = path.first().map(|entry| entry.0).unwrap_or(0);
        self.mark_cell_control_dirty(section_idx, parent_para_idx, outer_ctrl);
        self.document.sections[section_idx].raw_stream = None;
        self.rebuild_section(section_idx);
        self.event_log.push(DocumentEvent::TableRowInserted {
        ...
        Ok(super::super::helpers::json_ok_with(&format!(
            "\"paraIdx\":{},\"controlIdx\":{},\"charOffset\":{}",
            path.last().map(|entry| entry.2).unwrap_or(0),
            ctrl_idx,
            char_offset + 1
        )))
    }
    ...
    /// 셀/중첩 셀 내부 커서 위치에 인라인 그림을 삽입한다.
    pub fn insert_picture_in_cell_by_path_native(
        &mut self,
        section_idx: usize,
        parent_para_idx: usize,
        path_json: &str,
        ...
        let available_width = {
            let cell = self.resolve_cell_by_path(section_idx, parent_para_idx, &path)?;
            let inner_width =
                cell.width as i32 - cell.padding.left as i32 - cell.padding.right as i32 - 566;
            inner_width.max(1200) as u32
        };

        let display_width = width.min(available_width).max(1);
        let display_height = if width > 0 && display_width < width {
            ((height as u64 * display_width as u64) / width as u64).max(1) as u32
        } else {
            height.max(1)
        };
        ...
        let common_attr: u32 = 0x01 | (2 << 3) | (2 << 8) | (4 << 15) | (2 << 18);
        let common = CommonObjAttr {
            ctrl_id: 0x67736F20,
            attr: common_attr,
            treat_as_char: true,
        ...
        let border_x = [0i32, 0, display_width as i32, 0];
        let border_y = [
    ...
    fn build_inline_table_control(
    ...
    ) -> Result<(crate::model::table::Table, u32), HwpError> {
        use crate::model::paragraph::{CharShapeRef, LineSeg};
        use crate::model::style::{BorderFill, BorderLine, BorderLineType, DiagonalLine, Fill};
        use crate::model::table::{Cell, Table, TablePageBreak};

        let col_width = (content_width / col_count as u32).max(200);
        let total_width = col_width * col_count as u32;
        let cell_pad = crate::model::Padding {
            left: 510,
            right: 510,
        ...
        table.rebuild_grid();
        Ok((table, total_height))
    }

    pub(crate) fn insert_inline_control_into_paragraph(
        para: &mut Paragraph,
        control: Control,
        char_offset: usize,
        control_height: u32,
        ...
        if let Some(seg) = para.line_segs.first_mut() {
            let next_height = (control_height as i32).max(seg.line_height);
            if next_height > seg.line_height {
                seg.line_height = next_height;
                seg.text_height = next_height;
                seg.baseline_distance = (next_height as f64 * 0.85) as i32;
            }
        }
        ctrl_idx
    }
